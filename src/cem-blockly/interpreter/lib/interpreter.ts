...
import { InterpreterConstructor } from './types';
...
var Interpreter = function (code, opt_initFunc) {
  if (typeof code === 'string') {
    code = parse(code, Interpreter.PARSE_OPTIONS);
  }
  this.ast = code;
  ...
  for (const methodName in this) {
    if ( // extract step processing functions
  ...
  // Run the polyfills.
  this.ast = parse(this.polyfills_.join('\n'), Interpreter.PARSE_OPTIONS);
...
Interpreter.prototype.initFunction = function (scope) {
  ...
  wrapper = function (var_args) {
      ...
      for (let i = 0; i < args.length; i++) {
        ...
        if (!identifierRegexp.test(name)) {
          thisInterpreter.throwException(
            thisInterpreter.SYNTAX_ERROR, `Invalid function argument: ${name}`,
          );
        }
      }
      argsStr = args.join(', ');
    ...
    try {
      var ast = parse(
        `(function(${argsStr}) {${code}})`, Interpreter.PARSE_OPTIONS,
      );
    } catch (e) {
      // Acorn threw a SyntaxError.  Rethrow as a trappable error.
      thisInterpreter.throwException(
        thisInterpreter.SYNTAX_ERROR, `Invalid code: ${e.message}`,
      );
    }
    if (ast.body.length !== 1) {
      // Function('a', 'return a + 6;}; {alert(1);');
      thisInterpreter.throwException(
        thisInterpreter.SYNTAX_ERROR, 'Invalid code in function body.',
      );
    }
    newFunc.node = ast.body[0].expression;
    thisInterpreter.setProperty(
      newFunc, 'length', newFunc.node.length, Interpreter.READONLY_NONENUMERABLE_DESCRIPTOR,
    );
    return newFunc;
  };
  ...
  this.setProperty(
    this.FUNCTION, 'prototype', this.FUNCTION_PROTO, Interpreter.NONENUMERABLE_DESCRIPTOR,
  );
  this.FUNCTION.nativeFunc = wrapper;

  // Configure Function.prototype.
  this.setProperty(
    this.FUNCTION_PROTO, 'constructor', this.FUNCTION, Interpreter.NONENUMERABLE_DESCRIPTOR,
  );
  this.FUNCTION_PROTO.nativeFunc = function () { };
  this.FUNCTION_PROTO.nativeFunc.id = this.functionCounter_++;
  this.setProperty(
    this.FUNCTION_PROTO, 'length', 0, Interpreter.READONLY_NONENUMERABLE_DESCRIPTOR,
  );
  ...
  wrapper = function (thisArg, args) {
    ...
    if (args !== null && args !== undefined) {
      if (args.isObject) {
        ...
        thisInterpreter.throwException(
          thisInterpreter.TYPE_ERROR, 'CreateListFromArrayLike called on non-object',
        );
      }
    }
...
Interpreter.prototype.initObject = function (scope) {
  ...
  this.setProperty(
    this.OBJECT, 'prototype', this.OBJECT_PROTO, Interpreter.NONENUMERABLE_DESCRIPTOR,
  );
  this.setProperty(
    this.OBJECT_PROTO, 'constructor', this.OBJECT, Interpreter.NONENUMERABLE_DESCRIPTOR,
  );
  ...
  const throwIfNullUndefined = function (value) {
    if (value === undefined || value === null) {
      thisInterpreter.throwException(
        thisInterpreter.TYPE_ERROR, `Cannot convert '${value}' to object`,
      );
    }
  };
  ...
  wrapper = function (obj) {
    ...
    const props = obj.isObject ? obj.properties : obj;
  ...
  wrapper = function (proto) {
    ...
    if (proto === undefined || !proto.isObject) {
      thisInterpreter.throwException(
        thisInterpreter.TYPE_ERROR, 'Object prototype may only be an Object or null',
      );
    }
  ...
  wrapper = function (obj, prop) {
    if (!obj || !obj.isObject) {
      thisInterpreter.throwException(
        thisInterpreter.TYPE_ERROR, 'Object.getOwnPropertyDescriptor called on non-object',
      );
    }
    prop = String(prop);
    ...
    delete descriptor.value;
    const pseudoDescriptor = thisInterpreter.nativeToPseudo(descriptor);
    if (hasValue) {
  ...
  this.setNativeFunctionPrototype(
    this.OBJECT, 'toString', Interpreter.Object.prototype.toString,
  );
  this.setNativeFunctionPrototype(
    this.OBJECT, 'toLocaleString', Interpreter.Object.prototype.toString,
  );
  this.setNativeFunctionPrototype(
    this.OBJECT, 'valueOf', Interpreter.Object.prototype.valueOf,
  );
...
Interpreter.prototype.initString = function (scope) {
  ...
  const functions = [
    ...
    'slice', 'substr', 'substring', 'toLocaleLowerCase',
...
Interpreter.prototype.initDate = function (scope) {
  ...
  wrapper = function (value, var_args) {
    if (!thisInterpreter.calledWithNew()) {
      // Called as Date().
      // Calling Date() as a function returns a string, no arguments are needed.
      return Date();
    }
    // Called as new Date().
...
Interpreter.prototype.initRegExp = function (scope) {
  ...
  wrapper = function (string, callback) {
    ...
    const regexp = thisPseudoRegExp.data;
...
Interpreter.prototype.initError = function (scope) {
  ...
  this.ERROR = this.createNativeFunction(function (opt_message) {
    ...
    if (opt_message) {
      thisInterpreter.setProperty(
        newError, 'message', String(opt_message), Interpreter.NONENUMERABLE_DESCRIPTOR,
      );
    }
    return newError;
  }, true);
  ...
  this.setProperty(
    this.ERROR.properties.prototype, 'message', '', Interpreter.NONENUMERABLE_DESCRIPTOR,
  );
  this.setProperty(
    this.ERROR.properties.prototype, 'name', 'Error', Interpreter.NONENUMERABLE_DESCRIPTOR,
  );
...
Interpreter.prototype.initMath = function (scope) {
  ...
  for (var i = 0; i < mathConsts.length; i++) {
    this.setProperty(
      myMath, mathConsts[i], Math[mathConsts[i]], Interpreter.READONLY_NONENUMERABLE_DESCRIPTOR,
    );
  }
  const numFunctions = [
 ...
 * @param {Interpreter.Object} constructor Parent constructor function, or null if scope object.
...
Interpreter.prototype.createNativeFunction = function (nativeFunc, opt_constructor) {
  ...
  this.setProperty(
    func, 'length', nativeFunc.length, Interpreter.READONLY_NONENUMERABLE_DESCRIPTOR,
  );
  if (opt_constructor) {
    this.setProperty(
      func.properties.prototype, 'constructor', func, Interpreter.NONENUMERABLE_DESCRIPTOR,
    );
...
Interpreter.prototype.createAsyncFunction = function (asyncFunc) {
  ...
  this.setProperty(
    func, 'length', asyncFunc.length, Interpreter.READONLY_NONENUMERABLE_DESCRIPTOR,
  );
  return func;
};
...
Interpreter.prototype.nativeToPseudo = function (nativeObj) {
  if ( // nativeObject is not a native object
    ...
    const wrapper = function () {
      return interpreter.nativeToPseudo(
        nativeObj.apply(interpreter,
          Array.prototype.slice.call(arguments).map((i) => interpreter.pseudoToNative(i))),
      );
    };
...
Interpreter.prototype.arrayNativeToPseudo = function (nativeArray) {
  const pseudoArray = this.createObjectProto(this.ARRAY_PROTO);
...
Interpreter.prototype.getProperty = function (obj, name) {
  ...
  return undefined;
  <-------*/
  if (obj.IS_SYS_OBJECT) {
    if (obj[name]) {
      return obj[name];
    }
    // return undefined;
    this.throwException(this.TYPE_ERROR, `Cannot find ${name}`);
  } else {
    do {
      if (obj.properties && name in obj.properties) {
...
Interpreter.prototype.setProperty = function (obj, name, value, opt_descriptor) {
  ...
  if (!obj.isObject) {
    if (strict) {
      this.throwException(
        this.TYPE_ERROR, `Can't create property '${name}' on '${obj}'`,
      );
    }
    return;
  }
  ...
  if (obj.preventExtensions && !(name in obj.properties)) {
    if (strict) {
      this.throwException(
        this.TYPE_ERROR, `Can't add property '${name}', object is not extensible`,
      );
    }
    return;
  }
...
Interpreter.prototype.createScope = function (node, parentScope) {
  const scope = this.createObjectProto(null);
 ...
 * @param {!Interpreter.Object} parentScope Scope to link to.
 * @param {Interpreter.Object=} opt_scope Optional object to transform into scope.
 * @return {!Interpreter.Object} New scope.
 */
Interpreter.prototype.createSpecialScope = function (parentScope, opt_scope) {
  ...
  const scope = opt_scope || this.createObjectProto(null);
  scope.parentScope = parentScope;
...
Interpreter.prototype.populateScope_ = function (node, scope) {
  if (node.type === 'VariableDeclaration') {
    for (var i = 0; i < node.declarations.length; i++) {
      this.setProperty(
        scope, node.declarations[i].id.name, undefined, Interpreter.VARIABLE_DESCRIPTOR,
      );
    }
...
Interpreter.prototype.unwind = function (type, value, label) {
  ...
  loop: for (let stack = this.stateStack; stack.length > 0; stack.pop()) {
    ...
    if (type === Interpreter.Completion.BREAK) {
      const isValid = label
        ? (state.labels && state.labels.indexOf(label) !== -1) : (state.isLoop || state.isSwitch);
      if (isValid) {
        stack.pop();
        return;
      }
      ...
      if (
        label ? (state.labels && state.labels.indexOf(label) !== -1) : state.isLoop
      ) {
        return;
      }
    }
...
Interpreter.prototype.createSetter_ = function (func, left, value) {
  ...
  const funcThis = Array.isArray(left) ? left[0] : this.global;
  const node = new this.nodeConstructor({
    options: {},
  });
  node.type = 'CallExpression';
...
Interpreter.prototype.stepCallExpression = function (stack, state, node) {
  ...
  if (!state.doneExec_) {
    ...
    <-------*/
    if (!state.funcThis_.IS_SYS_OBJECT) {
      if (func === undefined) {
        this.throwException(this.TYPE_ERROR, `${node.callee.property.name} is not a function`);
      }
      if (!func || !func.isObject) {
        this.throwException(this.TYPE_ERROR, `${func} is not a function`);
      }
    ...
    if (funcNode) {
      ...
      const argsList = this.createObjectProto(this.ARRAY_PROTO);
      ...
      if (typeof code !== 'string') {
        ...
        try {
          var ast = parse(String(code), Interpreter.PARSE_OPTIONS);
        } catch (e) {
          // Acorn threw a SyntaxError.  Rethrow as a trappable error.
          this.throwException(this.SYNTAX_ERROR, `Invalid code: ${e.message}`);
        }
        const evalNode = new this.nodeConstructor({
          options: {},
        });
      ...
      // ADDITION(by visubong):S //////////////////////////////////////////////////////////////////////
    } else if (state.funcThis_.IS_SYS_OBJECT) {
      state.value = func.apply(state.funcThis_, state.arguments_);
      // ADDITION(by visubong):E //////////////////////////////////////////////////////////////////////
    } else {
  ...
  if (!state.doneInit_) {
    ...
    if (node.left.declarations
      ...
      if (state.scope.strict) {
        this.throwException(
          this.SYNTAX_ERROR, 'for-in loop variable declaration may not have an initializer.',
        );
      }
...
Interpreter.prototype.stepLogicalExpression = function (stack, state, node) {
  ...
  if (!state.doneRight_) {
    if (
      (node.operator === '&&' && !state.value) || (node.operator === '||' && state.value)
    ) {
      // Shortcut evaluation.
      stack.pop();
      stack[stack.length - 1].value = state.value;
  ...
  if (node.operator === '-') {
    ...
    if (Array.isArray(value)) {
      ...
      try {
        ...
        if (state.scope.strict) {
          this.throwException(
            this.TYPE_ERROR, `Cannot delete property '${name}' of '${obj}'`,
          );
        } else {
          result = false;
        }
...
Interpreter.prototype.stepVariableDeclaration = function (stack, state, node) {
  ...
  while (declarationNode) {
    // Skip any declarations that are not initialized.
    // They have already been defined as undefined in populateScope_.
...
export default Interpreter as InterpreterConstructor;